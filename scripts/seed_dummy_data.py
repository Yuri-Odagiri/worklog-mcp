...
async def seed_dummy_data(project_name: str, days: int = 7, entries_per_day: int = 10):
    ...
    if not db_path.exists():
        logger.error(f"エラー: データベース {db_path} が存在しません。")
        logger.error("先に init_db.py を実行してください。")
        return
    ...
    for user_data in DUMMY_USERS:
        ...
        avatar_path = await generate_gradient_avatar(
            user_data["theme_color"], user_data["user_id"], project_context
        )
        ...
        logger.info(
            f"  ✓ {user.name} ({user.user_id}) - アバター: {Path(avatar_path).name}"
        )
    ...
    for day in range(days):
        ...
        for user_data in DUMMY_USERS:
            ...
            for hour in range(entries_per_day):
                ...
                # 進捗表示
                if entry_count % 50 == 0:
                    logger.info(f"  {entry_count} エントリー作成済み...")
    ...
    for user_data in DUMMY_USERS:
        ...
        logger.info(f"{user_data['name']} ({user_id}):")
        logger.info(f"  - 総投稿数: {stats['total_posts']}")
        logger.info(f"  - 今日の投稿数: {stats['today_posts']}")
        if stats["first_post"]:
            logger.info(f"  - 初投稿: {stats['first_post']}")
            logger.info(f"  - 最終投稿: {stats['latest_post']}")
    logger.info("\nダミーデータの投入が完了しました！")
...
def main():
    ...
    parser.add_argument(
        "--days", "-d", type=int, default=7, help="生成する日数 (デフォルト: 7)"
    )
    parser.add_argument(
        "--entries-per-day",
        "-e",
