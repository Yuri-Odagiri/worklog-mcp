...
# 既存プロセスのチェック
EXISTING_PROCS=$(ps aux | grep -E "(worklog_mcp|job_worker_daemon)" | grep -v grep)
...
echo ""
if [ "$MCP_OK" = true ] && [ "$WEB_OK" = true ] && [ "$JOB_OK" = true ]; then
    ...
    echo "  📊 ログ確認: tail -f $MCP_LOG $WEB_LOG $JOB_LOG"
else
